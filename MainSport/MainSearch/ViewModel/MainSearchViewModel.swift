...
nonisolated enum MainSearchViewState: Equatable, Sendable {

    case idle
    case waitingForInput(query: String, minimumCharacterCount: Int)
    case loading(query: String)
    case loaded(MainSearchPresentation)
...
final class MainSearchViewModel {
    ...
    @Published private(set) var state: MainSearchViewState = .idle
    ...
    private let searchService: MainSearchServicing
    ...
    init(
        ...
        searchService: MainSearchServicing
    ) {
    ...
    private func searchTeams(matching query: String) async {
        ...
        do {
            ...
            AppLogger.logUIError(
                error,
                message: "Main search loading failed",
    ...
    private func makePresentation(
        from teams: [MainSearchTeam],
        query: String
    ) -> MainSearchPresentation {
        MainSearchPresentation(
            query: query,
    ...
    private func makeTeamViewData(from team: MainSearchTeam) -> MainSearchTeamViewData {
        MainSearchTeamViewData(
            id: team.id,
            name: team.name,
            subtitle: makeSubtitle(from: team),
            logoURL: team.logoURL,
            fallbackSystemImageName: "shield"
        )
    }

    private func makeSubtitle(from team: MainSearchTeam) -> String {
        let values = [
            team.city,
            team.countryName,
            team.code
